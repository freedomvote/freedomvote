...
def v1(request):
    questions = [
        ...
        for x in Question.objects.all().order_by("id")
    ]
    ...
    categories = [
        ...
        for x in Category.objects.all().order_by("id")
    ]
    ...
    for x in Politician.objects.all().order_by("id"):
        if Statistic.objects.filter(politician=x).exists():
            ...
            if x.party:
                p["party"] = {"name": x.party.name, "shortname": x.party.shortname}
            elif x.party_other:
                p["party"] = {"name": x.party_other, "shortname": None}
            else:
                p["party"] = None
    ...
    return JsonResponse(
        {"politicians": politicians, "questions": questions, "categories": categories}
    )
