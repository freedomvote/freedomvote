...
def candidates_view(request):
    ...
    categories = (
        Category.objects.filter(statistic__id__gt=0).order_by("name").distinct()
    )
    ...
    if has_cookie:
        ...
        context["share_url"] = "{0}/share?{1}".format(
            settings.BASE_URL, formatted_query_params
        )
...
def compare_view(request):
    ...
    if request.method == "POST":
        ...
        for category in categories:
            ...
            session_statistics["category_%d" % category.id] = 10 - sum(values) / float(
                len(cq)
            )
    ...
    if request.method == "GET":
        response = render(
            request, "core/compare/index.html", {"data": data, "meta": default_meta}
        )
...
def politician_view(request, politician_id):
    politician = get_object_or_404(
        Politician.objects.filter(statistic__id__gt=0).distinct(), pk=politician_id
    )

    answers = Answer.objects.filter(politician=politician).order_by(
    ...
    embed_url_reverse = reverse(
        "politician_statistic_spider_embed", kwargs={"politician_id": politician_id}
    )
...
def politician_statistic_spider_view(request, politician_id):
    ...
    return JsonResponse(
        {"categories": [s.category.name for s in statistics], "values": values}
    )
...
def politician_statistic_spider_view_embed(request, politician_id):
    ...
    politician_url_reverse = reverse(
        "politician", kwargs={"politician_id": politician_id}
    )
    ...
    return render(
        ...
        {"politician_id": politician_id, "politician_url": politician_url_absolute},
    )
...
def politician_statistic_view(request, politician_id):
    ...
    if "evaluate" in request.GET:
        ...
        if category_id:
            val = 10 - (
                sum(delta_by_cat[category_id]) / float(len(delta_by_cat[category_id]))
            )
            pos.append(val)
            neg.append(10 - val)

        summary = {"titles": titles, "values": {"positive": pos, "negative": neg}}
        ...
        detail = {"categories": [s.category.name for s in statistics], "values": values}
...
def politician_edit_profile_view(request, unique_key):
    ...
    return render(
        ...
        {"politician": politician, "form": form, "links": links},
    )
...
def politician_edit_questions_view(request, unique_key):
    ...
    return render(
        ...
        {"politician": politician, "questions": questions, "answers": answers},
    )
...
def politician_publish_view(request, unique_key):
    ...
    for category in categories:
        ...
        if answers:
            ...
            value = int(answers_agg["agreement_level__sum"] / answers.count() * 10)
            stat, created = Statistic.objects.get_or_create(
                politician=politician, category=category, defaults={"value": value}
            )
            stat.value = value
            stat.save()

    return JsonResponse(
        {"type": "success", "text": force_text(_("answers_published_successfully"))}
    )
...
def politician_unpublish_view(request, unique_key):
    ...
    return JsonResponse(
        {"type": "success", "text": force_text(_("answers_unpublished_successfully"))}
    )
...
def politician_link_add_view(request, unique_key):
    ...
    if not error:
        l = Link(politician=politician, url=url)
        l.save()
        url = ""
    ...
    return render(
        request,
        "core/edit/links.html",
        {"links": links, "politician": politician, "error": error, "input": url},
    )
...
def politician_link_delete_view(request, unique_key, link_id):
    ...
    return render(
        request, "core/edit/links.html", {"links": links, "politician": politician}
    )
...
def party_politician_add_view(request, party_name):
    ...
    return render(
        request, "core/party/politician_edit.html", {"politician": None, "form": form}
    )
...
def party_export_view(request, party_name):
    ...
    for p in politicians:
        ...
        writer.writerow([c.encode("latin1") if c is not None else "" for c in cols])
...
def party_politician_delete_view(request, party_name, politician_id):
    ...
    messages.success(
        request,
        _("politician_delete_success") % (politician.first_name, politician.last_name),
    )
...
class PoliticianRegistrationView(FormView):
    ...
    def send_mail(self, politician):
        profile_url = reverse(
            "politician_edit_profile", kwargs={"unique_key": politician.unique_key}
        )
